...
class LabelIndexView(ListView):
	...
	def get_queryset(self):
		blog_list = BlogPost.objects.current().filter(labels__slug__exact=self.kwargs.get("label"))
		foto_list = FotoPost.objects.current().filter(labels__slug__exact=self.kwargs.get("label"))
